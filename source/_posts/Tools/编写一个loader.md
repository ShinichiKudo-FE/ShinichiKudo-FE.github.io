...
所以，在你开发一个 Loader 时，请 **保持其职责的单一性**，你只需 **关心输入和输出**。
<!-- more -->
...
曾介绍过 `ResolveLoader` 用于配置 Webpack 如何寻找 Loader。 默认情况下只会去 `node_modules` 目录下寻找，为了让 Webpack 加载放在本地项目中的 Loader 需要修改 `resolveLoader.modules`。
